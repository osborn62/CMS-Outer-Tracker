...
#badbumps, badbumpsNonEdge, badbumpsVCal3, badbumpsVCal5 = cal.BumpBonding(filename=path+mapsaid+"_BumpBonding", print_out=False, printout =False, returnAll = True, show_plot=0)
#badbumpsv2, badbumpsNonEdgev2, badbumpsVCal3v2, badbumpsVCal5v2 = cal.BumpBonding(filename=path+mapsaid+"_BumpBondingv2", print_out=False, printout =False, returnAll = True, show_plot=0)
#    def BumpBonding(self, s_type="CAL", n_pulse=1000, ref_val = 250, trim_ampl = -1, rbr = 0, show_plot = 1, save_plot = 1, print_out =False, returnAll = False, printout = True, filename="../Results_MPATesting/bumpbonding_"):
        #scurvebb, thbb, noisebb = self.single_scurve( n_pulse = n_pulse, s_type = s_type, rbr = rbr, ref_val = ref_val, row = self.conf.rowsnom, col = self.conf.colsnom, step = 1, start = 0, stop = 256, pulse_delay = 500, extract_val = extract_val, extract = 1, plot = show_plot, print_file =0,israw=False, printout = printout )
            #CSV.ArrayToCSV (data_array, str(filename) + "_" + s_type + ".csv")
            #th_array, noise_array, notfittedpixels = self.plot_extract_scurve(row = np.array([row]), col = np.array([col]), s_type = s_type, scurve = data_array , n_pulse = n_pulse, nominal_DAC = extract_val, start = start, stop = stop, plot = plot, extract = extract, israw = False, printout = printout)
